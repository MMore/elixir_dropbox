defmodule ElixirDropboxTest do
  ...
  test "list folder contents", state do
    ...
    ["file1.txt", "file2.txt", "file1.pdf", "file2.pdf"]
    |> Enum.each(fn fln ->
      ElixirDropbox.Files.upload(state[:client], "/upload_test/#{fln}", "test/fixtures/#{fln}")
    end)

    assert ["/upload_test/file1.txt", "/upload_test/file2.txt"] ==
             ElixirDropbox.Files.list_filenames_in_folder(state[:client], "/upload_test", "txt")

    assert ["/upload_test/file1.pdf", "/upload_test/file2.pdf"] ==
             ElixirDropbox.Files.list_filenames_in_folder(state[:client], "/upload_test", "pdf")
  end
  ...
  test "delete folder", state do
    ElixirDropbox.Files.create_folder(state[:client], "/deleted_folder")
